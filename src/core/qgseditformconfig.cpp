...
void QgsEditFormConfig::readXml( const QDomNode& node )
{
  ...
  {
    mInitCode = editFormInitCodeNode.toElement().text();
  }
  ...
  {
    mUseInitCode = editFormInitUseCodeNode.toElement().text().toInt();
  }
...
void QgsEditFormConfig::writeXml( QDomNode& node ) const
{
  ...
  efiucField.appendChild( doc.createTextNode( useInitCode() ? "1" : "0" ) );
