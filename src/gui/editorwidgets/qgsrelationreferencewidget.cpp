    ...
    , mAllowMapIdentification( false )
{
  ...
  {
    connect( mParentAttributeDialog, SIGNAL( finished( int ) ), this, SLOT( deleteHighlight() ) );
  }
}
...
void QgsRelationReferenceWidget::deleteMessageBarItem()
{
  QgsMessageBar* messageBar = mEditorContext.messageBar();
  if ( !messageBar || !mMessageBarItem )
    return;
...
void QgsRelationReferenceWidget::highlightFeature( bool scale )
{
  ...
  QgsMapCanvas* canvas = mEditorContext.mapCanvas();
  if ( !canvas )
    return;
  ...
  // highlight
  deleteHighlight();
  mHighlight = new QgsHighlight( canvas, feat, mReferencedLayer );
  QSettings settings;
...
void QgsRelationReferenceWidget::mapIdentification()
{
  ...
  if ( messageBar )
  {
    ...
    QString msg = tr( "identify a feature of %1 to be associated" ).arg( mReferencedLayer->name() );
    mMessageBarItem = QgsMessageBar::createMessage( title, msg );
    messageBar->pushItem( mMessageBarItem );
    connect( mMapTool, SIGNAL( deactivated() ), this, SLOT( deleteMessageBarItem() ) );
  }
}
